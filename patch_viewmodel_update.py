...
init_block = """    private val likedTrackDao = AppDatabase.getDatabase(application).likedTrackDao()
    val likedSongs: StateFlow<List<LikedTrack>> = likedTrackDao.getAllLikedTracks()
    ...
    fun checkForUpdates() {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                // Ignorar errores de red para no molestar al usuario si no hay internet o el repo no existe
                e.printStackTrace()
            }
        }
    }
